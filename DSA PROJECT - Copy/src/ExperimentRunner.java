...
public class ExperimentRunner {
    // Usage:
    //   java -cp "bin:libs/sqlite-jdbc.jar" ExperimentRunner ug.db seeds/locations.csv
    public static void main(String[] args) throws Exception {
        ...
        DBLoader loader = new DBLoader(dbFile);
            // Load location IDs and names from CSV (locationId,name,area,type,lat,lon)
    ...
    // Collision study (vary load factor)
    ...
    for (HashTable.Mode mode : new HashTable.Mode[]{HashTable.Mode.CHAINING, HashTable.Mode.LINEAR_PROBING}) {
        for (double lf : loadTargets) {
            ...
            // Optionally print a small summary line
